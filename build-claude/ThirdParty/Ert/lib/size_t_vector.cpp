...
static void size_t_vector_realloc_data__(size_t_vector_type * vector , int new_alloc_size, size_t default_value) {
...
static void size_t_vector_memmove(size_t_vector_type * vector , int offset , int shift) {
  ...
  if ((shift + vector->size > vector->alloc_size))
    size_t_vector_realloc_data__( vector , util_int_min( 2*vector->alloc_size , shift + vector->size ), vector->default_value);
...
static size_t_vector_type * size_t_vector_alloc__(int init_size , size_t default_value, size_t * data, int alloc_size , bool data_owner ) {
  ...
  */
  ...
  if (init_size > 0)
    size_t_vector_resize( vector , init_size , default_value );  /* Filling up the init size elements with the default value */
  ...
  if (new_size > vector->size) {
    if (new_size > vector->alloc_size) {
      for (int i = vector->size; i < vector->alloc_size; i++)
        vector->data[i] = default_value;
      size_t_vector_realloc_data__( vector, 2 * new_size, default_value);
    }
    else
...
void size_t_vector_memcpy_from_data( size_t_vector_type * target , const size_t * src , int src_size ) {
  ...
  size_t_vector_resize( target, src_size, 0);
...
size_t_vector_type * size_t_vector_alloc_copy( const size_t_vector_type * src) {
  ...
  size_t_vector_realloc_data__( copy , src->alloc_size, src->default_value );
  copy->size = src->size;
  ...
  size_t_vector_assert_writable( vector );
  {
      ...
      util_abort("%s: Sorry - can NOT set negative indices. called with index:%d \n",__func__ , index);
    {
      if (vector->alloc_size <= index)
        size_t_vector_realloc_data__(vector , 2 * (index + 1), vector->default_value);  /* Must have ( + 1) here to ensure we are not doing 2*0 */
...
void size_t_vector_append(size_t_vector_type * vector , size_t value) {
  ...
  size_t_vector_resize(vector, size+1, 0);
...
void size_t_vector_free_data(size_t_vector_type * vector) {
  ...
  size_t_vector_realloc_data__(vector , 0, vector->default_value);
}
...
void size_t_vector_set_many(size_t_vector_type * vector , int index , const size_t * data , int length) {
  size_t_vector_assert_writable( vector );
  {
    ...
    if (min_size > vector->alloc_size)
      size_t_vector_realloc_data__(vector , 2 * min_size, vector->default_value);
...
bool size_t_vector_init_linear(size_t_vector_type * vector , size_t start_value, size_t end_value, int num_values) {
  ...
  size_t_vector_reset( vector );
  size_t_vector_iset( vector, 0 , start_value);
  {
    double slope = (end_value - start_value) / (num_values - 1);

    for (int i=1; i < num_values - 1; i++) {
      size_t value = (size_t) start_value + slope*i;
      size_t_vector_iset(vector, i, value);
    }
  }
  size_t_vector_iset( vector, num_values - 1, end_value);
  return true;
}
...
void size_t_vector_shrink(size_t_vector_type * vector) {
  size_t_vector_realloc_data__(vector , vector->size, vector->default_value);
}
...
void size_t_vector_fread_data( size_t_vector_type * vector , int size, FILE * stream) {
  size_t_vector_realloc_data__( vector , size, vector->default_value );
