...
static void bool_vector_realloc_data__(bool_vector_type * vector , int new_alloc_size, bool default_value) {
...
static void bool_vector_memmove(bool_vector_type * vector , int offset , int shift) {
  ...
  if ((shift + vector->size > vector->alloc_size))
    bool_vector_realloc_data__( vector , util_int_min( 2*vector->alloc_size , shift + vector->size ), vector->default_value);
...
static bool_vector_type * bool_vector_alloc__(int init_size , bool default_value, bool * data, int alloc_size , bool data_owner ) {
  ...
  */
  ...
  if (init_size > 0)
    bool_vector_resize( vector , init_size , default_value );  /* Filling up the init size elements with the default value */
  ...
  bool_vector_assert_writable( vector );
  {
      ...
      util_abort("%s: Sorry - can NOT set negative indices. called with index:%d \n",__func__ , index);
    {
      if (vector->alloc_size <= index)
        bool_vector_realloc_data__(vector , 2 * (index + 1), vector->default_value);  /* Must have ( + 1) here to ensure we are not doing 2*0 */
...
void bool_vector_set_many(bool_vector_type * vector , int index , const bool * data , int length) {
  bool_vector_assert_writable( vector );
  {
    ...
    if (min_size > vector->alloc_size)
      bool_vector_realloc_data__(vector , 2 * min_size, vector->default_value);
