...
static void int_vector_realloc_data__(int_vector_type * vector , int new_alloc_size, int default_value) {
...
static void int_vector_memmove(int_vector_type * vector , int offset , int shift) {
  ...
  if ((shift + vector->size > vector->alloc_size))
    int_vector_realloc_data__( vector , util_int_min( 2*vector->alloc_size , shift + vector->size ), vector->default_value);
...
static int_vector_type * int_vector_alloc__(int init_size , int default_value, int * data, int alloc_size , bool data_owner ) {
  ...
  */
  ...
  if (init_size > 0)
    int_vector_resize( vector , init_size , default_value );  /* Filling up the init size elements with the default value */
  ...
  int_vector_assert_writable( vector );
  {
      ...
      util_abort("%s: Sorry - can NOT set negative indices. called with index:%d \n",__func__ , index);
    {
      if (vector->alloc_size <= index)
        int_vector_realloc_data__(vector , 2 * (index + 1), vector->default_value);  /* Must have ( + 1) here to ensure we are not doing 2*0 */
...
void int_vector_set_many(int_vector_type * vector , int index , const int * data , int length) {
  int_vector_assert_writable( vector );
  {
    ...
    if (min_size > vector->alloc_size)
      int_vector_realloc_data__(vector , 2 * min_size, vector->default_value);
