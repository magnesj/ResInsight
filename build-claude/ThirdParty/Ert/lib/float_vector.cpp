...
static void float_vector_realloc_data__(float_vector_type * vector , int new_alloc_size, float default_value) {
...
static void float_vector_memmove(float_vector_type * vector , int offset , int shift) {
  ...
  if ((shift + vector->size > vector->alloc_size))
    float_vector_realloc_data__( vector , util_int_min( 2*vector->alloc_size , shift + vector->size ), vector->default_value);
...
static float_vector_type * float_vector_alloc__(int init_size , float default_value, float * data, int alloc_size , bool data_owner ) {
  ...
  */
  ...
  if (init_size > 0)
    float_vector_resize( vector , init_size , default_value );  /* Filling up the init size elements with the default value */
  ...
  if (new_size > vector->size) {
    if (new_size > vector->alloc_size) {
      for (int i = vector->size; i < vector->alloc_size; i++)
        vector->data[i] = default_value;
      float_vector_realloc_data__( vector, 2 * new_size, default_value);
    }
    else
...
void float_vector_memcpy_from_data( float_vector_type * target , const float * src , int src_size ) {
  ...
  float_vector_resize( target, src_size, 0);
...
float_vector_type * float_vector_alloc_copy( const float_vector_type * src) {
  ...
  float_vector_realloc_data__( copy , src->alloc_size, src->default_value );
  copy->size = src->size;
  ...
  float_vector_assert_writable( vector );
  {
      ...
      util_abort("%s: Sorry - can NOT set negative indices. called with index:%d \n",__func__ , index);
    {
      if (vector->alloc_size <= index)
        float_vector_realloc_data__(vector , 2 * (index + 1), vector->default_value);  /* Must have ( + 1) here to ensure we are not doing 2*0 */
...
void float_vector_append(float_vector_type * vector , float value) {
  ...
  float_vector_resize(vector, size+1, 0);
...
void float_vector_free_data(float_vector_type * vector) {
  ...
  float_vector_realloc_data__(vector , 0, vector->default_value);
}
...
void float_vector_set_many(float_vector_type * vector , int index , const float * data , int length) {
  float_vector_assert_writable( vector );
  {
    ...
    if (min_size > vector->alloc_size)
      float_vector_realloc_data__(vector , 2 * min_size, vector->default_value);
...
bool float_vector_init_linear(float_vector_type * vector , float start_value, float end_value, int num_values) {
  ...
  float_vector_reset( vector );
  float_vector_iset( vector, 0 , start_value);
  {
    double slope = (end_value - start_value) / (num_values - 1);

    for (int i=1; i < num_values - 1; i++) {
      float value = (float) start_value + slope*i;
      float_vector_iset(vector, i, value);
    }
  }
  float_vector_iset( vector, num_values - 1, end_value);
  return true;
}
...
void float_vector_shrink(float_vector_type * vector) {
  float_vector_realloc_data__(vector , vector->size, vector->default_value);
}
...
void float_vector_fread_data( float_vector_type * vector , int size, FILE * stream) {
  float_vector_realloc_data__( vector , size, vector->default_value );
