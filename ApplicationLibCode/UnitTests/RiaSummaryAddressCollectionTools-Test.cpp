...
TEST( RiaSummaryAddressCollectionTools, BuildCurveDefsPreservesCasePointer )
{
    ...
    auto dummyCasePtr = reinterpret_cast<RimSummaryCase*>( static_cast<uintptr_t>( 0xDEAD ) );

    auto wellAddress = RifEclipseSummaryAddress::wellAddress( "WOPT", "WELL-A" );

    RiaSummaryCurveDefinition              curveDef( dummyCasePtr, wellAddress, false );
    std::vector<RiaSummaryCurveDefinition> sourceDefs = { curveDef };
    ...
    EXPECT_EQ( 1u, result.size() );
    EXPECT_EQ( dummyCasePtr, result[0].summaryCaseY() );
}
...
{
    auto caseA = reinterpret_cast<RimSummaryCase*>( static_cast<uintptr_t>( 0x1000 ) );
    auto caseB = reinterpret_cast<RimSummaryCase*>( static_cast<uintptr_t>( 0x2000 ) );
