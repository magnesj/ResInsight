...
public:
        ...
        : RigMainGrid()
    {
        ...
        double startAngle = 0.0, endAngle = M_PI / 4.0;
        
        for (size_t k = 0; k < 2; ++k) {
            double z = k * 10.0;
            
            for (size_t j = 0; j < 3; ++j) {
                double radius = innerRadius + j * (outerRadius - innerRadius) / 2.0;
                
                for (size_t i = 0; i < 3; ++i) {
                    double angle = startAngle + i * (endAngle - startAngle) / 2.0;
...
TEST(RigCylindricalGrid, CylindricalCoordinateExtraction)
{
    ...
    bool success = grid.getCylindricalCoords(cellIndex, innerRadius, outerRadius, 
                                            startAngle, endAngle, topZ, bottomZ);
...
TEST(RigCylindricalGrid, InvalidCellIndexHandling)
{
    ...
    // Test with invalid cell index
    size_t invalidCellIndex = grid.cellCount() + 100;
    bool success = grid.getCylindricalCoords(invalidCellIndex, innerRadius, outerRadius,
                                            startAngle, endAngle, topZ, bottomZ);
...
TEST(RigCylindricalGrid, AngleRangeValidation)
{
    ...
    for (size_t cellIdx = 0; cellIdx < grid.cellCount(); ++cellIdx) {
        double innerRadius, outerRadius, startAngle, endAngle, topZ, bottomZ;
        bool success = grid.getCylindricalCoords(cellIdx, innerRadius, outerRadius,
                                                startAngle, endAngle, topZ, bottomZ);
        
        if (success) {
            EXPECT_GE(endAngle, startAngle);
            EXPECT_GE(startAngle, -M_PI);
            EXPECT_LE(endAngle, M_PI);
