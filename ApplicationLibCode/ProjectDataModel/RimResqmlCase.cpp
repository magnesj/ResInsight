...
CAF_PDM_SOURCE_INIT( RimResqmlCase, "ResqmlCase", "RimResqmlCase" );

//--------------------------------------------------------------------------------------------------
///
//--------------------------------------------------------------------------------------------------
RimResqmlCase::RimResqmlCase()
    : RimEclipseCase()
{
    CAF_PDM_InitScriptableObject( "RimResqmlCase", ":/EclipseInput48x48.png" );
}
...
///
//--------------------------------------------------------------------------------------------------
bool RimResqmlCase::openEclipseGridFile()
{
    if ( eclipseCaseData() )
    {
    ...
    const QString fileName = gridFileName();

    if ( eclipseCaseData()->mainGrid()->cellCount() == 0 )
    {
        QString errorMessages;
        if ( RifResqmlFileTools::openGridFile( fileName, eclipseCaseData(), &errorMessages ) )
        {
            QFileInfo gridFileInfo( fileName );
            setCaseUserDescription( gridFileInfo.completeBaseName() );
    ...
    // Read properties from the RESQML file
    RifResqmlFileTools::createInputProperties( fileName, eclipseCaseData() );
...
///
//--------------------------------------------------------------------------------------------------
void RimResqmlCase::defineUiOrdering( QString uiConfigName, caf::PdmUiOrdering& uiOrdering )
{
...
///
//--------------------------------------------------------------------------------------------------
QString RimResqmlCase::locationOnDisc() const
{
...
///
//--------------------------------------------------------------------------------------------------
bool RimResqmlCase::importAsciiInputProperties( const QStringList& fileNames )
{
