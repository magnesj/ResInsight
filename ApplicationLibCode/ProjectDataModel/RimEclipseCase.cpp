...
void RimEclipseCase::setReservoirData( RigEclipseCaseData* eclipseCase )
{
    ...
    if ( eclipseCaseData() )
    {
        ...
        auto completionTypeCallback = [this]( std::vector<double>& completionTypeResult, size_t timeStep )
        {
            ...
            for ( const QString propertyName : { "PERMX", "PERMY", "PERMZ" } )
            {
                for ( auto userCalculation : RimProject::current()->gridCalculationCollection()->calculations() )
                {
                    if ( auto gridCalculation = dynamic_cast<RimGridCalculation*>( userCalculation ) )
                    {
                        ...
                        if ( std::find( outputCases.begin(), outputCases.end(), this ) == outputCases.end() ) continue;
                    }
