...
void RigCaseCellResultsData::computeCompletionTypeForTimeStep( size_t timeStep )
{
    ...
    if ( m_computeCompletionTypeCallback )
    {
        m_computeCompletionTypeCallback( completionTypeResult, timeStep );
    }
}
                                                              ...
                                                              std::vector<RigCaseCellResultsData*> destinationResults )
{
    ...
    RigCaseCellResultsData* mainCaseResults = mainCaseResultsData->results( poroModel );

    for ( auto* cellResultsStorage : destinationResults )
    {
        if ( !cellResultsStorage ) continue;
        if ( cellResultsStorage == mainCaseResults ) continue; // Do not copy on top of itself
