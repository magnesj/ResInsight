...
void RicMswUnifiedData::addWellData(RicMswTableData wellData)
{
    ...
    auto it = std::find_if(m_wellDataList.begin(), m_wellDataList.end(),
                          [&wellName](const RicMswTableData& data) {
                              return data.wellName() == wellName;
                          });
...
{
    return std::any_of(m_wellDataList.begin(), m_wellDataList.end(),
                       [](const RicMswTableData& wellData) {
                           return wellData.hasLgrData();
                       });
}
...
std::vector<QString> RicMswUnifiedData::wellNames() const
{
    ...
    std::transform(m_wellDataList.begin(), m_wellDataList.end(),
                   std::back_inserter(names),
                   [](const RicMswTableData& wellData) {
                       return wellData.wellName();
                   });
