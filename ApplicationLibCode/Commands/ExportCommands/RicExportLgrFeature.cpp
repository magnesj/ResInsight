    ...
    {
        {
            ...
            formatter.add( lgrInfo.lgrGridSize().i() );
            formatter.add( lgrInfo.lgrGridSize().j() );
            formatter.add( lgrInfo.lgrGridSize().k() );
...
std::map<QString, std::vector<LgrInfo>> RicExportLgrFeature::createLgrInfoListForTemporaryLgrs( const RigMainGrid* mainGrid )
{
    ...
    for ( size_t i = 0; i < mainGrid->gridCount(); i++ )
    {
        ...
        auto mainGridCellCountI = mainGridBoundingBox.second.i() - mainGridBoundingBox.first.i() + 1;
        ...
        auto refinementI = grid->cellCountI() / mainGridCellCountI;
        auto refinementJ = grid->cellCountJ() / mainGridCellCountJ;
        auto refinementK = grid->cellCountK() / mainGridCellCountK;
