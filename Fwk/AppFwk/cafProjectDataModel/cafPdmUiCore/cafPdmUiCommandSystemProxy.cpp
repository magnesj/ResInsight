...
std::vector<PdmFieldHandle*> PdmUiCommandSystemProxy::fieldsFromSelection( PdmFieldHandle* editorField )
{
    ...
    // Find all selected items to be able to modify multiple rows in the table editor
    ...
    for ( auto& item : items )
    {
        ...
        if ( objectHandle )
        {
            ...
            else
            {
                ...
                std::vector<PdmFieldHandle*> childFields = objectHandle->fields();
                for ( auto field : childFields )
                {
                    std::vector<PdmObjectHandle*> childObjects = field->children();
