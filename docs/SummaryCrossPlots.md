...
When a time step **T** in input vector **A** is not found in input vector **B**, ResInsight interpolates between the time steps before and after **T** in vector **B**. In this way, ResInsight calculates an interpolated value for time step **T** in vector **B**. If either the value at the current time step or one of the values involved in interpolation is a **NULL** value, the resulting vector will have a **NULL** value at the current time step.
