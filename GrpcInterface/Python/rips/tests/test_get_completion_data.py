...
def test_get_completion_data_stability_comprehensive(rips_instance, initialize_test):
    ...
    validate_compdat_properties(data_run1.compdat, expected_diameter=0.25, expected_skin=0.1)
    ...
    for compdat in data_run1.compdat:
        ...
        if compdat.HasField("skin_factor"):
            assert abs(compdat.skin_factor - 0.1) < 0.001, "Skin factor should match input"
        if compdat.HasField("start_md") and compdat.HasField("end_md"):
            assert 2190.0 <= compdat.start_md <= 2610.0, "Start MD should be in expected range"
            assert 2190.0 <= compdat.end_md <= 2610.0, "End MD should be in expected range"
...
def test_completion_data_regression_reference(rips_instance, initialize_test):
    ...
    if reference_file.exists():
        ...
        assert len(current_results["compdat"]) > 0, "Should have COMPDAT entries in reference"
        assert len(current_results["welspecs"]) > 0, "Should have WELSPECS entries in reference"
...
def validate_compdat_properties(compdat_entries, expected_diameter=None, expected_skin=None):
    ...
    for i, compdat in enumerate(compdat_entries):
        ...
        assert compdat.lower_k >= compdat.upper_k, f"Entry {i}: Lower K should be >= upper K"
        assert compdat.open_shut_flag in ["OPEN", "SHUT"], f"Entry {i}: Invalid open/shut flag"
        
        # Validate optional fields when present
        if compdat.HasField("transmissibility"):
            assert compdat.transmissibility >= 0, f"Entry {i}: Transmissibility should be non-negative"
        
        if compdat.HasField("diameter") and expected_diameter is not None:
            assert abs(compdat.diameter - expected_diameter) < 0.001, f"Entry {i}: Diameter mismatch"
            
        if compdat.HasField("skin_factor") and expected_skin is not None:
            assert abs(compdat.skin_factor - expected_skin) < 0.001, f"Entry {i}: Skin factor mismatch"
            
        if compdat.HasField("start_md") and compdat.HasField("end_md"):
            assert compdat.end_md >= compdat.start_md, f"Entry {i}: End MD should be >= start MD"
...
def test_completion_data_hash_cross_platform_stability(rips_instance, initialize_test):
    ...
    json_str1 = json.dumps(test_data, sort_keys=True, separators=(',', ':'), ensure_ascii=True)
    json_str2 = json.dumps(test_data, sort_keys=True, separators=(',', ':'), ensure_ascii=True)
...
def test_completion_data_cross_validation(rips_instance, initialize_test):
    ...
    perf_interval = well_path.append_perforation_interval(start_md, end_md, diameter, skin_factor)
    ...
    for compdat in completion_data.compdat:
        ...
        if compdat.HasField("diameter"):
            if abs(compdat.diameter - diameter) < 0.001:
                ...
                if compdat.HasField("skin_factor"):
                    assert abs(compdat.skin_factor - skin_factor) < 0.001, "Skin factor should match"
                    
                # Check MD range
                if compdat.HasField("start_md") and compdat.HasField("end_md"):
                    assert start_md <= compdat.start_md <= end_md + 5, "Start MD should be in range"
                    assert start_md <= compdat.end_md <= end_md + 5, "End MD should be in range"
